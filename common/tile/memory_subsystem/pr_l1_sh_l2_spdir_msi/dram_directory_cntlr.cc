...
void
SparseDirectoryCntlr::handleMsgFromL2Cache(tile_id_t sender, ShmemMsg* shmem_msg)
{
   // add synchronization cost
   if (sender == _memory_manager->getTile()->getId()){
      getShmemPerfModel()->incrCurrTime(_sparse_directory_cache->getSynchronizationDelay(L2_CACHE));
   }
   else{
...
void
SparseDirectoryCntlr::processNextReqFromL2Cache(IntPtr address)
{
...
DirectoryEntry*
SparseDirectoryCntlr::processDirectoryEntryAllocationReq(ShmemReq* shmem_req)
{
...
void
SparseDirectoryCntlr::processNullifyReq(ShmemReq* shmem_req)
{
   IntPtr address = shmem_req->getShmemMsg()->getAddress();
   tile_id_t requester = shmem_req->getShmemMsg()->getRequester();
...
void
SparseDirectoryCntlr::processExReqFromL2Cache(ShmemReq* shmem_req, Byte* cached_data_buf)
{
   IntPtr address = shmem_req->getShmemMsg()->getAddress();
   tile_id_t requester = shmem_req->getShmemMsg()->getRequester();
   bool msg_modeled = shmem_req->getShmemMsg()->isModeled();
...
void
SparseDirectoryCntlr::processShReqFromL2Cache(ShmemReq* shmem_req, Byte* cached_data_buf)
{
   IntPtr address = shmem_req->getShmemMsg()->getAddress();
   tile_id_t requester = shmem_req->getShmemMsg()->getRequester();
   bool msg_modeled = shmem_req->getShmemMsg()->isModeled();
...
void
SparseDirectoryCntlr::retrieveDataAndSendToL2Cache(ShmemMsg::Type reply_msg_type,
...
void
SparseDirectoryCntlr::processInvRepFromL2Cache(tile_id_t sender, ShmemMsg* shmem_msg)
{
   IntPtr address = shmem_msg->getAddress();
...
void
SparseDirectoryCntlr::processFlushRepFromL2Cache(tile_id_t sender, ShmemMsg* shmem_msg)
{
   IntPtr address = shmem_msg->getAddress();
...
void
SparseDirectoryCntlr::processWbRepFromL2Cache(tile_id_t sender, ShmemMsg* shmem_msg)
{
   IntPtr address = shmem_msg->getAddress();
...
void
SparseDirectoryCntlr::sendDataToDram(IntPtr address, Byte* data_buf, bool modeled)
{
   // Write data to Dram
...
UInt32
SparseDirectoryCntlr::getCacheLineSize()
{
   return _memory_manager->getCacheLineSize();
}

ShmemPerfModel*
SparseDirectoryCntlr::getShmemPerfModel()
{
